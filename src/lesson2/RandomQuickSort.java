 ...
 * 随机快排
 * 思想
 * 基于快排思想，只是将基准value 改为随机选取当前数组段
 * 为了避免数据情况影响排序算法复杂度
 * 算法的复杂度是基于概率期望的值O（logn）
 */
public class RandomQuickSort {
    ...
    private void sort(int[] array, int index1, int index2) {
        ...
        int valueIndex=index1+(int)(Math.random()*(index2-index1+1));//基准value 改为随机选取当前数组段
        int[] partion = partion(array, index1, index2, array[valueIndex]);
